...
- 15 app tests passed; TypeScript checking and Python compilation passed. The durable outbox test covers an offline manual entry followed by a note, including server ID reconciliation. A temporary owner then exercised the restarted real API and worker: cached BCD resolved in 2.14 seconds to 3575 Wilshire at $0 provider cost; the test owner was erased afterwards (`runtime-smoke.json`).
...
- The final iOS Release **build 21** succeeded after dependency restoration, and app/extension code signatures verified. Both app and share extension report build 21; the archived build and JavaScript bundle hashes are recorded in `verification.json`. The bundled config points to the phone-facing local API. The Release bundle contains the new recovery copy and excludes the “Save diagnostics” view. The queue-only native share extension remains in the app group.
