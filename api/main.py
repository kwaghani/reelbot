...
@app.get('/readyz')
def ready():
    """Dependency diagnostics. Render must poll /healthz instead."""
    result = {'database': {'healthy': False}, 'storage': {'healthy': False}, 'queue': {'healthy': False}}
    try:
        with connect() as conn:
            conn.execute('select 1')
            queued = conn.execute("select count(*) as n from jobs where status in ('queued','processing')").fetchone()['n']
        result['database'] = {'healthy': True}
        result['queue'] = {'healthy': True, 'depth': queued}
    except Exception as exc:
        LOG.error('readyz_database_failed type=%s', type(exc).__name__)
        result['database']['error'] = type(exc).__name__
        result['queue']['error'] = 'database_unavailable'
    try:
        from worker.storage import exists
        # A non-existent sentinel still exercises R2's authenticated HEAD operation.
        exists('diagnostics/readyz-sentinel')
        result['storage'] = {'healthy': True, 'mode': 'r2' if settings().has_r2 else 'local_degraded'}
    except Exception as exc:
