...
p = Xcodeproj::Project.new(File.join(dir, 'ImageryUI.xcodeproj'))
t = p.new_target(:ui_test_bundle, 'ImageryUITests', :ios, '15.1')
source = File.expand_path('../app/tests/ImageryUITests.swift', __dir__)
...
s.add_test_target(t)
s.save_as(p.path, 'ImageryUITests', true)
