...
p = Xcodeproj::Project.new(File.join(dir, 'VenueUI.xcodeproj'))
t = p.new_target(:ui_test_bundle, 'VenueUITests', :ios, '15.1')
source = File.expand_path('../app/tests/VenueUITests.swift', __dir__)
t.add_file_references([p.main_group.new_file(source)])
t.build_configurations.each do |c|
  c.build_settings.merge!({
    'SWIFT_VERSION' => '5.0', 'PRODUCT_BUNDLE_IDENTIFIER' => 'com.krishwaghani.reelbot.venueuitests',
...
s.add_test_target(t)
s.save_as(p.path, 'VenueUITests', true)
