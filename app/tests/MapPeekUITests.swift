...
final class ReelBotMotionReview: XCTestCase {
 ...
 func testE_MapMarkerSheet() {
  ...
  if !toggle.waitForExistence(timeout:1) {
   ...
   let motion=settings.staticTexts["Motion"].firstMatch;XCTAssertTrue(motion.waitForExistence(timeout:5));motion.tap()
  }
  ...
  defer { settings.activate(); if toggle.value as? String != original { toggle.coordinate(withNormalizedOffset:CGVector(dx:0.9,dy:0.5)).tap();Thread.sleep(forTimeInterval:1) }; app.activate() }
  ...
  XCTAssertTrue(app.buttons["Directions"].waitForExistence(timeout: 10)); XCTAssertTrue(app.buttons["Open place"].exists); XCTAssertFalse(app.buttons["Hours & ratings"].exists); capture("Peek 190 points")
  let handle=app.descendants(matching:.any).matching(identifier:"Map sheet handle").firstMatch
  XCTAssertTrue(handle.exists)
  XCTAssertEqual(handle.value as? String,"Peek")
  handle.coordinate(withNormalizedOffset:CGVector(dx:0.5,dy:0.5)).press(forDuration:0.2,thenDragTo:app.coordinate(withNormalizedOffset:CGVector(dx:0.5,dy:0.50)),withVelocity:.slow,thenHoldForDuration:0.1)
  pause();capture("Slow drag to medium")
  let mediumHandle=app.descendants(matching:.any).matching(identifier:"Map sheet handle").element(boundBy:0)
  XCTAssertEqual(mediumHandle.value as? String,"Medium")
  app.buttons["Open place"].tap();pause();capture("Full place detail")
  XCTAssertTrue(app.buttons["Edit"].exists)
  app.descendants(matching:.any).matching(identifier:"Map sheet handle").element(boundBy:0).coordinate(withNormalizedOffset:CGVector(dx:0.5,dy:0.5)).press(forDuration:0.2,thenDragTo:app.coordinate(withNormalizedOffset:CGVector(dx:0.5,dy:0.64)),withVelocity:.slow,thenHoldForDuration:0.1)
  pause();capture("Drag back toward peek")
  app.coordinate(withNormalizedOffset:CGVector(dx:0.15,dy:0.40)).tap();pause();capture("Map background dismissal")

 }
 func testH_MarkerSwitchAtPeekAndMedium() {
  app.terminate(); app.launchEnvironment = ["REELBOT_FIXTURE_COUNT":"16"]; app.launch()
  XCTAssertTrue(app.buttons["Search saved entries"].waitForExistence(timeout:10));tab("Map");pause()
  func marker(_ name:String)->XCUIElement { app.descendants(matching:.any).matching(identifier:name).firstMatch }
  func handle()->XCUIElement { app.descendants(matching:.any).matching(identifier:"Map sheet handle").element(boundBy:0) }
  XCTAssertTrue(marker("Tartine").waitForExistence(timeout:10));marker("Tartine").tap();pause()
  XCTAssertTrue(app.buttons["Expand Tartine"].exists);XCTAssertEqual(handle().value as? String,"Peek")
  XCTAssertTrue(marker("Dayglow").isHittable);marker("Dayglow").tap();pause()
  XCTAssertTrue(app.buttons["Expand Dayglow"].exists);XCTAssertEqual(handle().value as? String,"Peek");capture("Second marker at peek")
  app.buttons["Expand Dayglow"].tap();pause();XCTAssertEqual(handle().value as? String,"Medium")
  XCTAssertTrue(marker("The Broad").isHittable);marker("The Broad").tap();pause()
  XCTAssertTrue(app.buttons["Expand The Broad"].exists);XCTAssertEqual(handle().value as? String,"Medium");capture("Second marker at medium")
  app.coordinate(withNormalizedOffset:CGVector(dx:0.12,dy:0.40)).tap();pause();XCTAssertEqual(handle().value as? String,"Peek");capture("Backdrop returns to peek")
  app.coordinate(withNormalizedOffset:CGVector(dx:0.12,dy:0.40)).tap();pause();XCTAssertFalse(app.buttons["Open place"].exists);capture("Map background restores camera")
 }
...
final class ReelBotReducedMotionReview: XCTestCase {
 func testReducedMotionOnDevice() {
  ...
  app.descendants(matching:.any).matching(identifier:"Map").firstMatch.tap();Thread.sleep(forTimeInterval:1)
  let cluster=app.descendants(matching:.any).matching(identifier:"80 saved entries in San Francisco").firstMatch
  if cluster.waitForExistence(timeout:5){cluster.tap();Thread.sleep(forTimeInterval:1)}
  let pin=app.descendants(matching:.any).matching(identifier:"Tartine").firstMatch
  XCTAssertTrue(pin.waitForExistence(timeout:5));pin.tap();Thread.sleep(forTimeInterval:1)
  XCTAssertTrue(app.buttons["Directions"].exists)
  let peek=XCTAttachment(screenshot:XCUIScreen.main.screenshot());peek.name="Reduced motion map peek";peek.lifetime = .keepAlways;add(peek)
  app.buttons["Open place"].tap();Thread.sleep(forTimeInterval:1)
  let full=XCTAttachment(screenshot:XCUIScreen.main.screenshot());full.name="Reduced motion map full";full.lifetime = .keepAlways;add(full)
