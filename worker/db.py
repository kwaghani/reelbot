        ...
        with _pool_lock:
            if _pool is None:
                ...
                if not current.database_url:
                    raise ConfigurationError('Required configuration variable DATABASE_URL is missing')
...
def check_pool_capacity():
    """Fail fast when the planned API+worker connection ceiling is unsafe."""
    current = settings()
    ...
    LOG.info('database_pool pool_size=%s max_overflow=%s service_ceiling=%s planned_total=%s max_connections=%s',
             current.pool_size, current.max_overflow, current.pool_ceiling, planned, maximum)
    return {'max_connections': maximum, 'planned_total': planned, 'service_ceiling': current.pool_ceiling}
