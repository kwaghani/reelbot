...
def text_coverage(data):
    ...
    with tempfile.TemporaryDirectory(prefix='reelbot-cover-ocr-') as tmp:
        ...
        cmd = [settings().tesseract_command, str(path), 'stdout', '--psm', '11', 'tsv']
...
def store_thumb(data):
    ...
    put('thumbs/'+key+'.jpg', data, 'image/jpeg')
...
def google_bytes(candidate, stats):
    ...
    for width,height in [(360,450),(240,300)]:
        ...
        with _network_slots, httpx.Client(trust_env=False,timeout=5) as client:
            response=client.get('https://places.googleapis.com/v1/'+quote(name,safe='/')+'/media',params={'maxWidthPx':width,'maxHeightPx':height,'skipHttpRedirect':'true'},headers={'X-Goog-Api-Key':settings().google_maps_api_key or ''})
...
def commons_candidates(place, kind, stats):
    ...
    for page in json.loads(body).get('query',{}).get('pages',{}).values():
        ...
        if place.get('id'):
            from worker.storage import put
            put(f"photos/{place['id']}/commons-{digest(raw)}.jpg", raw, 'image/jpeg')
        ...
        for candidate in candidates:
            try:
                ...
                if candidate['source'] != 'google':
                    ...
                    public.update(uri=uri, thumbnail_bytes=len(thumb_data))
                else:
