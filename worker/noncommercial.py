...
def nominatim_search(conn,query,metrics):
    endpoint=settings().nominatim_url
    ...
    request=Request(endpoint+'?'+urlencode({'q':query,'format':'jsonv2','limit':5,'addressdetails':1,'namedetails':1}),headers={'User-Agent':settings().nominatim_user_agent,'Accept':'application/json'})
